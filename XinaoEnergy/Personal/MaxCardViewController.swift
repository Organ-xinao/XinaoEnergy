...
class MaxCardViewController: AddNavViewController {
    ...
    func createUI() {
        for (index, value) in  dataArray.enumerated() {
            ...
            label.frame = CGRect(x: (kScreenWidth-estimatedFrame.width)/2, y: CGFloat(260*index+120), width: estimatedFrame.width, height: 20)
            self.view.addSubview(label)
            
            let imageV = UIImageView(frame: CGRect(x: 90, y: CGFloat(260*index+150), width: 200, height: 200))
